...
class StateDiscriminator:
    ...
    def train(self, buffer_pos, buffer_neg, iterations=1, batch_size=256):
        ...
        for iter_i in range(iterations):
            ...
            state_e, action_e, next_state_e, reward_e, not_done_e, _ = buffer_pos.sample(batch_size)

            state_o, action_o, next_state_o, reward_o, not_done_o, _ = buffer_neg.sample(batch_size)


            d_e = self.discriminator(state_e)
            d_o = self.discriminator(state_o)
            ...
            target[:batch_size] = 1
            accuracy = (pseudo_label.reshape(-1) == target).sum() / len(pseudo_label)
    ...
    def weight_sample(self, state):

        self.discriminator.eval()

        pred_prob = self.discriminator(state)
